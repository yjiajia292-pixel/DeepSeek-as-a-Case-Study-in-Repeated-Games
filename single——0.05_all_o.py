...
# ==== 文件路径 ====
DETAILED_CSV = f"detailed_results_error_{ERROR_RATE:.2f}_all_oD.csv"  # 详细结果文件（合作率/收益）
SUMMARY_CSV = f"summary_results_error_{ERROR_RATE:.2f}_all_oD.csv"  # 汇总结果文件
# REASON_CSV = f"llm_reasons_error_{ERROR_RATE:.2f}_1_o.csv"         # 决策理由文件

# ==== DeepSeek API 配置 ====
client = OpenAI(
    api_key="sk-8f710d639f5e497998a3b136ffb9ca55",  # 替换为你的API Key
...
You will inform yourself and your opponent of all previous choices made.
Please decide whether to cooperate (C) or defect (D) in this round.
Your response must start with exactly one of the following two phrases:
- 'I choose to cooperate ...'
- 'I choose to defect ...'
You may only add brief rationale after the phrase if needed.
"""
...
# ==== LLM决策函数 ====
def llm_decision( history_opponent, attempt=1, max_retries=3):
    """
    ...
    prompt = f"Your opponent's move history: {', '.join(history_opponent) if history_opponent else 'None'}.\nYour response must start with exactly one of the following two phrases: 'I choose to cooperate ...' or 'I choose to defect ...'."
    ...
    try:
        ...
        if move_text.lower().startswith("i choose to cooperate"):
            ...
            time.sleep(5)
            return llm_decision(history_opponent, attempt + 1, max_retries)
...
def run_single_game(opponent_strategy, game_number):
    ...
    for round_num in range(1, N_ROUNDS + 1):
        ...
        move_llm = llm_decision(history_opp_noisy)
...
if __name__ == "__main__":
    ...
    opponent_strategies = ['WSLS','HardMajority', 'SoftMajority'] # 可扩展策略列表
